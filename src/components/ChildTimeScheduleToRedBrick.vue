  ...
  <div class="time-schedule-area">
    ...
    <div class="tables-area">
      <div class="time-schedule-table-area" v-for="(table, tableIndex) in splitTables" :key="tableIndex">
        <div class="vertical-line red-brick"></div>
        <div class="time-schedule-table">
          <div class="header red-brick">
            <div translate="no">出航時間</div>
            <div translate="no">運航状況</div>
            <div translate="no">詳細</div>
          </div>
          <div class="data red-brick-line" v-for="(row, rowIndex) in table" :key="rowIndex" :class="{'pastTime': this.isPastTime(row.departure_time)}">
...
export default {
  name: 'ChildTimeScheduleToPierRedBrick',
  props: {
